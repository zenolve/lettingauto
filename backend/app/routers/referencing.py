"""Referencing â€” Stage 5 UI backing for Paragon checks.
...
Three endpoints replace the "go edit Airtable manually" workflow:
  ...
  POST /api/properties/{property_id}/referencing/instruct
       ...
       reference number on file. Idempotent â€” a tenant already referenced is
  ...
  POST /api/properties/{property_id}/referencing/outcome
       ...
       so the Stage 5â†’6 gate can pass.
...
def referencing_state(
    ...
    # Tenant-level "referencing recorded" â€” true once we have a Pass / Fail /
    ...
    # Landlord approval is a *property-level* concept â€” it's the existing
...
async def instruct_referencing(
    ...
    # Stage gate â€” referencing is a Stage 5 action. Block earlier so the agent
    # can't accidentally instruct Paragon before an offer is recorded.
    from app.routers.forms import _require_stage  # local import â€” avoid cycles
...
def record_outcome(
    ...
    - ``tenant_id`` + ``outcome`` â†’ updates that tenant's `Referencing_Status`
      (and `Referencing_Recorded` if the field exists on the Tenants table).
    - ``landlord_approval_received`` â†’ writes the property-level
      `LL_Offer_Accepted` checkbox. (Consolidated 2026-05-16 â€” previously we
    ...
    if body.tenant_id and body.outcome:
        ...
        try:
            ...
            if "UNKNOWN_FIELD_NAME" in msg and "Referencing_Recorded" in msg:
                # Field not present â€” write just the status and warn.
                at.update(at.TableNames.TENANTS, body.tenant_id, {"Referencing_Status": body.outcome})
                warnings.append("Tenants.Referencing_Recorded checkbox not found â€” only the Status was saved.")
            else:
                raise

    # ---- Landlord approval â†’ Property.LL_Offer_Accepted ----------------
